...
def main():
    ...
    parser.add_argument('--src_dir', '-s', help='Path to ScanNet pickle data directory')
    ...
    parser.add_argument('--save_ply', '-p', help='Convert .pts to .ply', action='store_true')

    args = parser.parse_args()
    print(args)

    SRC_DIR = args.src_dir
    DST_DIR = args.dst_dir if args.dst_dir else './seg/'
    ...
    for dataset_idx, dataset in enumerate(datasets):
        filename = os.path.abspath(os.path.join(SRC_DIR, 'scannet_%s.pickle' % dataset))
                ...
                for block_idx in range(idx_last_non_empty_block + 1):
                    ...
                    for block_split_idx in range(block_split_num):
                        ...
                        if ((idx + 1) % batch_size == 0) \
                            ...
                            filename_h5 = os.path.join(DST_DIR, dataset, '%s_%d.h5' % (offset_name, idx_h5))
                            ...
                            #     filepath_label_ply = os.path.join(DST_DIR, dataset, 'ply_label',
